...
$id = $_GET['id'] ?? 0;
if (empty($id)) header("Location: plan-management.php");
...
$page_title = "Edit Plan: " . htmlspecialchars($plan['name']);
?>
...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark fw-bold"><i class="fas fa-edit me-2"></i>Edit Subscription Plan</h1>
            </div>
...
<div class="content">
    <div class="container-fluid">
        <form id="editPlanForm" class="row">
                ...
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    <div class="card-body p-4">
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" class="form-control form-control-lg rounded-3" value="<?= htmlspecialchars($plan['name']) ?>" required>
                        </div>
                        
                        <div class="mb-0">
                            <label class="form-label fw-bold">Terms & Conditions <span class="text-muted small">(Use rich text to format)</span></label>
                            <textarea name="terms" id="editor" class="form-control"><?= htmlspecialchars($plan['terms']) ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    <div class="card-header bg-white border-0 py-3">
                        <h6 class="card-title fw-bold mb-0">Update Pricing & Duration</h6>
                    </div>
                    <div class="card-body p-4">
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" name="price" class="form-control form-control-lg" value="<?= $plan['price'] ?>" required>
                            </div>
                        </div>
                        
                        <div class="mb-4">
                            <label class="form-label fw-bold">Duration Value <span class="text-danger">*</span></label>
                            <input type="number" name="duration_value" class="form-control form-control-lg" value="<?= $plan['duration_value'] ?>" required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="duration_unit" class="form-select form-select-lg" required>
                                <option value="days" <?= $plan['duration_unit'] === 'days' ? 'selected' : '' ?>>Days</option>
                                <option value="months" <?= $plan['duration_unit'] === 'months' ? 'selected' : '' ?>>Months</option>
                                <option value="years" <?= $plan['duration_unit'] === 'years' ? 'selected' : '' ?>>Years</option>
                            </select>
                        </div>
...
<script>
    ...
    document.getElementById('editPlanForm').onsubmit = function(e) {
        ...
        fetch('api/plan-actions.php?action=edit', {
            method: 'POST',
            body: formData
        })
        .then(r => r.json())
